...
主線の依頼は `/kaigi` から始まり、進行中の現在値は `blackboard` に出ます。
知識は `/library` から図書室に入り、まだ形になっていない断片は `/board` を通って `noticeboard` に置かれます。
それに対して `staffroom` は、薄氷をどう使い、どう考え、どう設計しているかを整理しておく場所です。世界観上は `準備室` として扱います。
